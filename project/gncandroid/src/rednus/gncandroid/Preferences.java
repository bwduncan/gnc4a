...
public class Preferences extends PreferenceActivity {
	...
	/*
	 * On create shows preferences from XML. Sets listner to file chooser as self
	 * and starts file chooser activity when clicked. Also shows, about, help and
	 * version dialogs.
	 * 
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == RESULT_FOR_FILE && resultCode == RESULT_OK) {
			...
			findPreference(pref_data_file_key).getEditor()
					.putString(pref_data_file_key, path).commit();
		} else
			super.onActivityResult(requestCode, resultCode, data);
	}
	/*
	 * This method is triggered when back button is pressed. So, if the user does
	 * not set all required parameters and tries to return to application, error
	 * message should be displayed and navigation should be cancelled.
	 * 
	...
	public void onBackPressed() {
		...
		if (!app.canReadData()) {
			...
			new AlertDialog.Builder(Preferences.this)
					.setTitle(R.string.error_prefs_not_set)
					.setMessage(R.string.error_message_prefs_not_set)
					.setPositiveButton(R.string.alert_dialog_ok,
							new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog, int whichButton) {
